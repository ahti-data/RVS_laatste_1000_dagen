...
calculate_costs_by_bin_size <- function(
    ...
    cost_columns,
    cost_date_col,
    bin_size = c("months33", "months5","months24", "50", "100", "1000", "2years", "last_month", "wlz_categorisation", "quarterly_2years"),
    aggregate_groupby_cols = NULL,
    id_col = c(),
    inflation_correction = NULL
    ){
  tictoc::tic("grid creation before overlap", quiet=F)
  # define bins
  ...
  if (chosen_bin_size == "last_month") {
    ...
    n_bins = 5
  } else if (chosen_bin_size %in% c("months24")) {
    n_bins = 24
  } else if (chosen_bin_size == "quarterly_2years") {
    n_bins = 8
  ...
  # splits, for efficiency
  rins_with_costs <- unique(costs_dt_clean$rinpersoon)
  
  dt_grid_match <- dt_grid[rinpersoon %in% rins_with_costs]
  dt_grid_nomatch <- dt_grid[!rinpersoon %in% rins_with_costs]
  
  # remove t=0 & t=1 for nomatch
  dt_grid_nomatch <- dt_grid_nomatch[!(died == "In leven" & t %in% c(0, 1))]
  dt_grid_nomatch <- dt_grid_nomatch[(died == "Overleden" & t %in% c(0, 1)), t := -1]
  
  if (chosen_bin_size == "wlz_categorisation") {
    dt_grid_match[, bin_end := fcase(
    ...
    dt_grid_match[, bin_end := gbadatumoverlijden + ((t + 1) * step_days)]
    dt_grid_match[, bin_start := gbadatumoverlijden + ((t) * step_days) + 1]
    
    # for overleden populations, we want to get until t=1, for alive, we want until -1
    dt_grid_match <- dt_grid_match[!(died == "In leven" & t %in% c(0, 1))]
    dt_grid_match <- dt_grid_match[(died == "Overleden" & t %in% c(0, 1)), t := -1]
  }
  
  # convert to numeric for foverlaps
  tictoc::tic("convert to numeric", quiet=F)
  dt_grid_match[, c("bin_start", "bin_end") := lapply(.SD, function(x) {
  ...
  setkey(dt_grid_match, rinpersoon, bin_start, bin_end)
  setkey(costs_dt_clean, rinpersoon, start_temp, end_temp)
  tictoc::toc()
  
  tictoc::tic("overlapping", quiet=F)

  print("overlapping")
  dt_merged_match <- foverlaps(
    x = dt_grid_match,
    y = costs_dt_clean,
    mult = "all",
    type = "any",
  ...
  dt_merged <- rbindlist(list(dt_merged_match, dt_grid_nomatch), fill=T)
  tictoc::toc()
  ...
  dt_merged[, c("bin_start", "bin_end", "start_temp", "end_temp") := NULL]
  setkey(dt_merged, rinpersoon)
...
load_reflijst_zorgproducten <- function(cols = NULL) {
  # load and clean reflijst
  reflijst_zorgproducten <- rio::import("K:/GezondheidWelzijn/MSZPRESTATIESVEKTTAB/ReflijstZorgproducten.csv")
  if (is.null(cols)) {reflijst_zorgproducten <- format_data(reflijst_zorgproducten, rin_num = F)}
  else {
    reflijst_zorgproducten <- format_data(reflijst_zorgproducten, rin_num = F)[, .SD, .SDcols = cols]
  }
  reflijst_zorgproducten[, mszdbczorgproduct := as.numeric(mszdbczorgproduct)]
  return(reflijst_zorgproducten)
}

load_reflijst_specialismediagnosecombinatie <- function(cols = NULL) {
  # load and clean reflijst
  reflijst_specialismediagnosecombinatie <- rio::import("K:/GezondheidWelzijn/MSZPRESTATIESVEKTTAB/ReflijstSpecialismeDiagnoseCombinatie.csv")
  if (is.null(cols)) {reflijst_specialismediagnosecombinatie <- format_data(reflijst_specialismediagnosecombinatie, rin_num = F)}
  else {
    reflijst_specialismediagnosecombinatie <- format_data(reflijst_specialismediagnosecombinatie, rin_num = F)[, .SD, .SDcols = cols]
  }
  return(reflijst_specialismediagnosecombinatie)
}

merge_with_validate <- function(x, y, by = intersect(names(x), names(y)),
                                by.x = by, by.y = by, validate = NULL, 
                                require_match = NULL, warn_on_type_mismatch = TRUE,
                                ...) {
  
  # 1. Check for missing values in merge keys
  # TODO: change to warning instead of stop
  for (key in by.x) {
    if(anyNA(x[[key]])) stop(sprintf("Missing values in left table key column '%s'", key))
  }
  for (key in by.y) {
    if(anyNA(y[[key]])) stop(sprintf("Missing values in right table key column '%s'", key))
  }
  
  # 2. Type check on merge keys
  # TODO: test whether redundant or not
  for (i in seq_along(by.x)) {
    if(class(x[[by.x[i]]])[1] != class(y[[by.y[i]]])[1]) {
      msg <- sprintf("Key column type mismatch: left '%s' is %s, right '%s' is %s",
                     by.x[i], class(x[[by.x[i]]])[1], by.y[i], class(y[[by.y[i]]])[1])
      if (warn_on_type_mismatch) warning(msg) else stop(msg)
    }
  }
  
  # 3. Warn on overlapping non-key columns
  overlap_cols <- setdiff(intersect(names(x), names(y)), by)
  if(length(overlap_cols) > 0) {
    warning(sprintf(
      "Overlapping non-key columns: $s. Suffixes will be added by merge().",
      paste(overlap_cols, collapse = ", ")
    ))
  }
  
  
  # 4. Validate merge type (using by-reference duplicate checking)
  # TODO: remove redundant stop if columns found
  if (!is.null(validate)) {
  ...
  # 5. Validate required matches
  # TODO: only check for non-NA keys?
  ...
  # TODO: test whether all args are being passed (e.g., all.x=T)
  
  # TODO: add optional verbose statement (nrows & percentage of rows/values matched)
