...
for (i in seq_along(rinpersoon_set_chunks)) {
  ...
  # NEW: create new diag cols for hartkleppen
  dt_mszprest_costs[, vektmszdbczorgproduct := as.numeric(vektmszdbczorgproduct)]
  
  dt_mszprest_costs[, ':=' (
    n_hartkleppen_totaal = as.integer(vektmszdbczorgproduct %in% c(979001188, 979001189, 979001190, 979001191, 979001192, 979001193)),
    n_hartkleppen_transkatheter = as.integer(vektmszdbczorgproduct %in% c(979001188, 979001189)),
    n_hartkleppen_klepchirurgie = as.integer(vektmszdbczorgproduct %in% c(979001190, 979001191, 979001192, 979001193))
  )]
  
  # remove cols
  dt_mszprest_costs[, c("vektmszdbczorgproduct") := NULL]
  
  dt_mszprest_monthly <- calculate_costs_by_bin_size(
    dt_mszprest_costs,
    dt_overlijden_with_matched[as.numeric(rinpersoon) %in% rinpersoon_set_chunk],
    cost_columns = c('vektmszvergoedbedragzvw', "n_hartkleppen_totaal", "n_hartkleppen_transkatheter", "n_hartkleppen_klepchirurgie"),
...
# only sav exists, and dataset is not very large, so faster to load in the whole dataset and filter by chunk later
...
#NEW: use chunking by rin instead of chunking by cols... takes way too long
n_chunks <- 10
rinpersoon_set_chunks <- split(rinpersoon_set, cut(seq_along(rinpersoon_set), n_chunks, labels = FALSE))

dt_msz_addon_monthly <- rbindlist(lapply(1:length(rinpersoon_set_chunks), function(chunk_i) {
  print(paste("chunk: ", chunk_i))
  rin_chunk <- rinpersoon_set_chunks[[chunk_i]]
  
  dt_msz_addon_chunk <- dt_msz_addon[rinpersoon %in% rin_chunk]
  

  for (cat in cats_to_split) {
    new_col_name <- paste0("bedrag_", cat)
    dt_msz_addon_chunk[, (new_col_name) := 0]
    dt_msz_addon_chunk[vektmszhoofdgroepsubgroepdg == cat, (new_col_name) := bedrag]
  }
  
  # Sum of oncology costs
  dt_msz_addon_chunk[, bedrag_13_01_88 := rowSums(.SD, na.rm = T), 
               .SDcols = c('bedrag_13_01', 'bedrag_13_02', 
                           'bedrag_13_03', 'bedrag_13_04', 
                           'bedrag_13_05', 'bedrag_13_06', 
                           'bedrag_13_07', 'bedrag_13_08',
                           'bedrag_13_88')]
  cost_cols <- grep("bedrag_", names(dt_msz_addon_chunk), value = T)
  
  # Count the number of declarations
  n_cols <- sub('bedrag_', 'n_', cost_cols)
  dt_msz_addon_chunk[, (n_cols) := lapply(.SD, function(x) 
    as.integer(x > 0)),
    .SDcols = cost_cols
  ]
  ...
  # create a lighter version of dt_overlijden, with only relevant rins and cols
  dt_overlijden_simplified_chunk <- dt_overlijden_with_matched[rinpersoon %in% rin_chunk]
  dt_overlijden_simplified_chunk <- dt_overlijden_simplified_chunk[, .SD, .SDcols = c("rinpersoon", "sample_id", "gbadatumoverlijden", "died", "cohort")]
  
  overlapped <- calculate_costs_by_bin_size(
    dt_msz_addon_chunk[, c('rinpersoon', 'vektmszdatumdg', 
                     costs_n),
                 with = F],
    dt_overlijden_simplified_chunk,
    cost_columns = costs_n,
    cost_date_col = "vektmszdatumdg",
    bin_size = "months33",
    aggregate_groupby_cols = names(dt_overlijden_simplified_chunk)
  )
  
  # remove the months from -24 to -33
  overlapped <- overlapped[!(t < -24)]
  
  # intermediate save
  arrow::write_parquet(overlapped, glue("data/processed/msz_addon_chunks/msz_addon_chunk_{chunk_i}"))
...
# add the overlijden cols back
dt_msz_addon_monthly <- merge(
  dt_msz_addon_monthly,
  dt_overlijden_with_matched,
  all.x=T
)
...
### NEW: HUISARTSDECLTAB ####
rinpersoon_set_2023 <- unique(dt_overlijden_with_matched[cohort=="2023"]$rinpersoon)

dt_huisarts <- load_dataset(2021:2024, "huisartsdecltab",
                       cols = c("rinpersoon", "hadeclbegindatumprest", 
                                "hadeclprestatiecode", "hadeclvergoedbedrag"),
                       rinpersoon_chunk = rinpersoon_set_2023)
stopifnot(any(dt_huisarts$rinpersoon < 100000000))

# set types
dt_huisarts[, hadeclprestatiecode := as.integer(hadeclprestatiecode)]

# create n_cols
acp_codes <- c(31244, 31381) # , 31420, 31242

# create ref_table for huisarts
dt_labels <- load_dataset(2023, "huisartsdecltab",
                          cols = c("hadeclprestatiecode"),
                          n_max = 0,
                          filetype = ".sav"
                          )
hadeclprestatiecode_labels <- attr(dt_labels$hadeclprestatiecode, "labels")
ref_table <- data.table(
  hadeclprestatiecode = trimws(as.character(hadeclprestatiecode_labels)),
  description = names(hadeclprestatiecode_labels)
)

ref_table[, is_consult := 
            grepl("^(12|13)", hadeclprestatiecode) &
            !grepl("^123", hadeclprestatiecode) &
            grepl("consult|visite", description, ignore.case =TRUE)]
ref_table[, hadeclprestatiecode := as.integer(hadeclprestatiecode)]

# save ref_table 
openxlsx2::write_xlsx(ref_table, "data/raw/ref_table_huisarts_consults_2023.xlsx")


# create n_consults column
consult_codes <- unique(ref_table[is_consult == T]$hadeclprestatiecode)

dt_huisarts[, n_consults := as.integer(hadeclprestatiecode %in% consult_codes)]
dt_huisarts[, n_acp := as.integer(hadeclprestatiecode %in% acp_codes)]

dt_huisarts_monthly <- calculate_costs_by_bin_size(
  dt_huisarts,
  dt_overlijden_with_matched[cohort == "2023"],
  cost_columns = c("hadeclvergoedbedrag", "n_consults", "n_acp"),
  cost_date_col = "hadeclbegindatumprest",
  bin_size = "quarterly_2years",
  aggregate_groupby_cols = names(dt_overlijden_with_matched)
)

# Save
setindex(dt_huisarts_monthly, NULL)
arrow::write_parquet(dt_huisarts_monthly, "data/processed/huisartsdecl_monthly.parquet")
rm(dt_huisarts_monthly, dt_huisarts)
gc()
