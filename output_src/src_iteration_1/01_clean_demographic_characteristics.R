...
df[, migratie_achtergrond := fcase(
  herkomst3 == "Nederlandse herkomst", 0L,
  herkomst3 %in% c("Kind van migrant", "Migrant"), 1L,
  default = "Onbekend"
)]
...
for (cohort in c(2013:2024)){
  ...
  df_doodoorz <- rbindlist(list(df_doodoorz, df), use.names = T)
  rm(df)
}
...
for (var_set in var_sets) {
  ...
  for (yr in years) {
    ...
    if (year_offset >= 2023) {
      file_path <- get_path_newest(
        path = "H:/data/Parquet_files_G_drive/Stapeling/parquet_files/",
      ...
      file_path <- get_path_newest(
        path = "G:/Maatwerk/STAPELINGSMONITOR/Geconverteerde bestanden/",
        string_pattern = year_offset,
        extension = ".parquet",
        method = "max_version"
      )
    ...
    df <- arrow::read_parquet(file_path, col_select = all_of(cols_available))
    setDT(df)
    ...
    replace_values_by_haven_labels(
      ...
      cols = grep(paste(cols_to_label, collapse = "|"),
                  names(df),
...
df <- df[, .(rinpersoon, year, hbopl, belanginkbronpers, inkomen_klasse)]
gc()
...
df[, belanginkbronpers_old := belanginkbronpers]
df[, belanginkbronpers := fcase(
  belanginkbronpers == "Werknemer",
  "Loon",
  belanginkbronpers %chin% c(
    "Directeur-grootaandeelhouder",
    "Zelfstandig ondernemer",
    "Overige zelfstandige"
  ),
  "Zelfstandige",
  belanginkbronpers == "Ontvanger bijstandsuitkering",
  "Bijstand",
  belanginkbronpers == "Ontvanger uitkering sociale voorziening overig",
  "Sociale voorzieningen",
  belanginkbronpers == "Ontvanger uitkering ziekte/arbeidsongeschiktheid",
  "Ziekte/AO",
  belanginkbronpers == "Ontvanger pensioenuitkering",
  "Pensioen",
  belanginkbronpers %chin% c(
  ...
  "Studiefinanciering",
  belanginkbronpers %chin% c(
    "Behoort tot huishouden zonder waargenomen inkomen",
    "Behoort tot huishouden zonder waargenomen inkomen/niet in populatie stapelingsmonitor (31-12-JJJJ)",
    "Ontvanger werkloosheidsuitkering",
    "Meewerkend gezinslid",
    "Overig zonder inkomen",
    "Onbekend, omdat de persoon niet tot de GBA-bevolking van 31-12-2016 behoort"
  ),
  "Overig",
  default = "Onbekend"
)]
stopifnot(uniqueN(df[belanginkbronpers == "Onbekend"]$belanginkbronpers_old) == 1)
df[, belanginkbronpers_old := NULL]
