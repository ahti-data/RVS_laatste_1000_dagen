...
for (yr in years) {
  ...
  # Save ZVW for 2023 for external checks
  if (yr == 2023){
    setindex(df, NULL)
    arrow::write_parquet(df, glue::glue("./data/processed/zvw_{yr}.parquet"))
  }
...
#### Prorate the costs ####
df <- r_parquet_get_dt("./data/raw/overlijden_with_matched.parquet")
...
cols_to_fill <- setdiff(names(df_binned), c(
  "rinpersoon", "gbadatumoverlijden", "cohort", "died", 'doodsoorzaak'))
setnafill(df_binned, fill = 0, cols = cols_to_fill)

setindex(df_binned, NULL)
arrow::write_parquet(df_binned, "./data/processed/zvw_1000_dagen.parquet")
rm(df, df_binned)
gc()
