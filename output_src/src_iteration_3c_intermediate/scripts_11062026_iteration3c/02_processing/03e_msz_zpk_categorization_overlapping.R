...
#### TODO ####
# REVIEW: check line 453 in "H:/_Personal_folders/Marco/loading function/loading_function.R"
...
dt_mszprest_OZP[, vektmszdeclaratiecode := as.numeric(vektmszdeclaratiecode)] # so merging with reflijst goes well
dt_mszprest_DBC <- dt_mszprest[substr(vektmszdeclaratiecode, 1, 2) %in% c("14", "15", "16", "17")]
dt_mszprest_DBC[, vektmszdeclaratiecode := NULL] # this column is currently useless for non-OZPs. will be replaced by activiteiten column
...
# for OZPs, we reduce the rows back to the n prestaties OZPs, and derive category based on ranking:
...
dt_mszprest_DBC <- dt_mszprest_DBC[, .(
  ...
  by = .(rinpersoon, vektmszbegindatumprest, vektmszdbczorgproduct, vektmszvergoedbedragzvw, vektmszsettingzpk, vektmszinstellingprest, id_prestatie)
]
