...
library(dplyr) # REVIEW; already loaded in inputs
...
for (i in 1) {
  ...
  for (yr in years) {
    ...
    dt[, ':='(
      ...
      heeft_aaa_0303 = as.integer(spec == '0303' & diag == '0405'),
      
      heeft_aaa_0406 = as.integer(spec == '0303' & diag == '0406'),
      
      heeft_heup_totaal = as.integer(
        (spec == '0303' & diag %in% c('0217', '0218', '0219', '0240')) |
          (spec == '0305' & diag %in% c('3017', '3018', '3019', '3020'))),
      
      heeft_heup_0303_0217 = as.integer(spec == '0303' & diag == '0217'),
      
      heeft_heup_0303_0218 = as.integer(spec == '0303' & diag == '0218'),
      
      heeft_heup_0303_0219 = as.integer(spec == '0303' & diag == '0219'),
      
      heeft_heup_0303_0240 = as.integer(spec == '0303' & diag == '0240'),
      
      heeft_heup_0305_3017 = as.integer(spec == '0305' & diag == '3017'),
      
      heeft_heup_0305_3018 = as.integer(spec == '0305' & diag == '3018'),
      
      heeft_heup_0305_3019 = as.integer(spec == '0305' & diag == '3019'),
      
      heeft_heup_0305_3020 = as.integer(spec == '0305' & diag == '3020')
      )]
    
    dt[, c('vektmszspecialismediagnosecombinatie', 'spec', 'diag') := NULL]
  ...
  dt_msz_prestatie <- merge(dt_msz_prestatie, dbc_names, all.x = T, by = c('vektmszdbczorgproduct'))
  
  # Check missings
  message('Share of missing names of DBC codes: ', 
          mean(ifelse(is.na(dt_msz_prestatie$vektmszdbczorgproduct_naam), 1, 0)))
  
  # Save the counts of DBC codes
  counts_list <- lapply(cols_to_keep, function(col) {
    dt_msz_prestatie[get(col) == 1,
                     .(n_totaal = uniqueN(rinpersoon)),
                     by = .(vektmszdbczorgproduct_naam)][order(-n_totaal)]
  })
  names(counts_list) <- cols_to_keep
  counts_list <- rbindlist(counts_list, use.names = T, idcol = T)
  dt_msz_prestatie$vektmszdbczorgproduct_naam <- NULL
  
  # Save DBC codes for heup and AAA
  openxlsx::write.xlsx(list(
    counts_list = counts_list
  ), file = './output/dbc_codes_heup_aaa.xlsx')
  ...
  for (yr in years) {
    ...
    # Split beeldvorming into first and second line 
