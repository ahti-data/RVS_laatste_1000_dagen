...
cols_to_select_stapeling_01_01 <- c(
  ...
  "percsm", "inkpers" # "belanginkbronpers", 
)
...
cols_to_select_msz_prestatie <- c('RINPERSOON', 
                                  ...
                                  'VEKTMSZZorgtype', 'VEKTMSZDeclaratiecode')
...
calculate_costs_by_bin_size <- function(
    ...
    cost_columns,
    cost_date_col,
    bin_size = c("months33", "months5", "50", "100", "1000", "last_month", "wlz_categorisation"),
...
load_mszprestaties <- function(yr, cols, rinpersoon_chunk = NULL, labelled_cols = NULL) {
  ...
  if (!is.null(rinpersoon_chunk)) {
    dt_mszprest <- ds |>
      filter(RINPERSOON %in% rinpersoon_chunk) |>
...
load_dataset <- function(
  ...
  # assertthat::assert_that(is.numeric(rinpersoon_chunk) | is.null(rinpersoon_chunk))
  ...
  dt_final <- rbindlist(lapply(years, function(yr) {
    ...
    filepath_sav <- get_path_newest_updated(folder_g, yr, ".sav", recursive = T)
    
    all_cols_uppercase <- tryCatch({
      names(haven::read_sav(filepath_sav, n_max = 0))
    }, error = function(e) {
      filepath_csv <- get_path_newest_updated(folder_g, yr, extension = ".csv", recursive = TRUE)
    ...
    if (filetype == ".parquet") {
      if (!any(grepl(".parquet", all_files))) {
        ...
        if (!is.null(rinpersoon_chunk)) {
          # find the rinpersoon column type
          class <- ds |>
            select("RINPERSOON") |>
            head(1) |>
            collect() |>
          ...
          if (class == "integer") {
            rinpersoon_chunk <- as.integer(rinpersoon_chunk)
          } else {
            rinpersoon_chunk <- as.character(rinpersoon_chunk)
          }
    ...
    if (filetype == ".csv") {
      if (!any(grepl(".csv", all_files))) {
        ...
        if (!is.null(rinpersoon_chunk)) {
          ...
          if (is.numeric(df$RINPERSOON)) {
            rinpersoon_chunk <- as.numeric(rinpersoon_chunk)
          } else {rinpersoon_chunk <- as.character(rinpersoon_chunk)}
    ...
    if (filetype == ".dta") {
      if (!any(grepl(".dta", all_files))) {
        ...
        filepath_dta <- get_path_newest_updated(folder_g, yr, extension = ".dta", recursive = TRUE)
