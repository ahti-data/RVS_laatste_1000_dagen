...
# Do external checks of the results of iteration 3 to iteration 1
...
dataset_configs <- list(
  ...
  # huisartsdecltab = list(
  #   filepath = './data/processed/huisartsdecl_monthly.parquet',
  #   demog_vars_to_split = c("age_cat", "provincie", "doodsoorzaak")
  # )
  # msz_prestatie_diagnostiek = list(
  #   filepath = './data/processed/msz_prestatie_monthly.parquet',
  #   demog_vars_to_split = c("doodsoorzaak", "age_cat")
  # ),
  msz_activit_diagnostiek = list(
    filepath = './data/processed/msz_activiteiten_monthly.parquet',
    demog_vars_to_split = c()
  )
  # msz_addon = list(
  #   filepath = './data/processed/msz_addon_monthly.parquet',
  #   demog_vars_to_split = c("age_cat")
  # ),
  # msz_addon_oncology_total = list(
  #   filepath = './data/processed/msz_addon_oncology_total_monthly.parquet',
  #   demog_vars_to_split = c("age_cat")
  # ),
  # msz_addon_oncology_total_cancer = list(
  #   filepath = './data/processed/msz_addon_oncology_total_cancer_monthly.parquet',
  #   demog_vars_to_split = c("age_cat")
  # ),
  # msz_addon_oncology_cancer = list(
  #   filepath = './data/processed/msz_addon_oncology_cancer_monthly.parquet',
  #   demog_vars_to_split = c("age_cat")
  # )
  # msz_prestaties = list(
  #   filepath = './data/processed/vektmszkosten_monthly.parquet',
  #   demog_vars_to_split = c("doodsoorzaak",
  #                           "age_cat",
  #                           "geslacht",
  #                           "inkomen_klasse",
  #                           # "seswoa_cat",
  ...
  #                           "burgstaat")
  # )
  # msz_prestaties_corrected = list(
  #   filepath = './data/processed/vektmszkosten_monthly_corrected.parquet',
  #   demog_vars_to_split = c()
  # ), 
  # wlz = list(
  ...
  # ), 
  # wlz_corrected = list(
  #   filepath = './data/processed/wlzkosten_monthly_corrected.parquet',
  #   demog_vars_to_split = c()
  # ), 
  # zvw = list(
  ...
  # ), 
  # zvw_corrected = list(
  #   filepath = './data/processed/zvw_1000_dagen_corrected.parquet',
  #   demog_vars_to_split = c()
  # )
)
...
aggregate_for_output <- function(dataset_name) {
  
  # extract configs 
  config <- dataset_configs[[dataset_name]]
  filepath <- config[["filepath"]]
  demog_vars_to_split <- config[["demog_vars_to_split"]]
  ...
  cost_columns <- setdiff(names(ds), c(
    c(group_cols_always, all_demog_cols),
    "rinpersoon",
    "sample_id",
    "gbadatumoverlijden",
    "t",
  ...
  col_names_other <- setdiff(names(ds), c(
    ...
    "burgstaat",
    "vektmszdbczorgproduct",
    "vektmszdbczorgproduct_naam", all_demog_cols))
  ...
  dt_dataset_total <- rbindlist(
    lapply(cost_column_chunks, apply_all_aggregations, 
           ds, col_names_other, group_cols_always, demog_vars_to_split, dataset_name)
  )
  
  # if msz_addon/hadeclhuisarts, rename to 24months
  if (grepl("addon|huisartsdecltab", dataset_name)) {
    dt_dataset_total[bin_size == "1000", bin_size := "24months"]
  }
  if (grepl("huisartsdecltab", dataset_name)) {
    dt_dataset_total[bin_size == "30", bin_size := "90"]
  }
...
apply_all_aggregations <- function(cost_columns_chunk, ds, 
                                   col_names_other = NULL, group_var_always, demog_vars_to_split, dataset_name) {
  
  if (!is.null(col_names_other)) {
    dt_dataset <- ds |>
      select(c(dplyr::all_of(c(cost_columns_chunk, demog_vars_to_split, col_names_other)))) |>
      collect()
  } else {
    dt_dataset <- ds |>
      # tail(100000) |>
  ...
  group_var_all <- c(group_var_always, demog_vars_to_split)
  ...
  if (length(demog_vars_to_split) > 0) {
    # first, set the values in the already aggregated dt to "all"
    dt_dataset_agg[, (demog_vars_to_split) := "all"]
    ...
    dt_dataset_agg_split_demog <- rbindlist(lapply(demog_vars_to_split, function(demog_col) {
      ...
      rename_cols <- c(setdiff(demog_vars_to_split, demog_col))
      dt[, (rename_cols) := "all"]
      return(dt)
    }), use.names=T)
...
#### apply and write ####
output_aggregations <- mapply(aggregate_for_output, names(dataset_configs), SIMPLIFY = FALSE)
