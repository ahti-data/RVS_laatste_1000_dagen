...
# rank based on costs as well as users for dbc top50
...
create_prestaties_aggregations_by_category <- function(dt, groupby_cols_always, cols_to_create_all) {
  ...
  add_n_totaal <- function(dt_aggregated, dt_original) {
    ...
    dt_population_sizes <- dt_overlijden_with_matched[, .(n_totaal_population = fnunique(sample_id)), by = actual_population_group_cols]
...
# Drop NA rows (people who had no matched prestaties) # REVIEW: I moved this to 03e; can remove
...
#### 3 NEW: Create top 50 tables for DBCs ####
# load in datasets
dt_mszprest_DBC_overlapped <- r_parquet_get_dt("data/processed/mszprest_DBC_overlapped.parquet")

# Drop NA rows (people who had no matched prestaties)
dt_mszprest_DBC_overlapped <- dt_mszprest_DBC_overlapped[!is.na(vektmszdbczorgproduct)]

# drop unnecessary cols
dt_mszprest_DBC_overlapped[, c("start_temp", "end_temp", 
                         "bin_end", "bin_start", "vektmszkoppelidprestza", "vektmszbeginjaarprest",
                         "gbadatumoverlijden") := NULL]

# oper_verr only
dt_mszprest_DBC_overlapped <- dt_mszprest_DBC_overlapped[zpk_category == "oper_verr"]

# first, create aggregations
dt_mszprest_DBC_agg_1000 <- create_prestaties_aggregations_by_category(
  dt_mszprest_DBC_overlapped,
  groupby_cols_always = c("cohort", "died", "vektmszdbczorgproduct"),
  cols_to_create_all = NULL
)[order(cohort, died)][,bin_size := "1000d"]

dt_mszprest_DBC_agg_30 <- create_prestaties_aggregations_by_category(
  dt_mszprest_DBC_overlapped[t == -1],
  groupby_cols_always = c("cohort", "died", "vektmszdbczorgproduct"),
  cols_to_create_all = NULL
)[order(cohort, died)][,bin_size := "30d"]

dt_mszprest_DBC_agg <- rbindlist(list(dt_mszprest_DBC_agg_1000, dt_mszprest_DBC_agg_30))
dt_mszprest_DBC_agg[died == "In leven", died := "In_leven"]
rm(dt_mszprest_DBC_agg_1000, dt_mszprest_DBC_agg_30)
gc()

ranking_variables <- c("n_totaal_gebruikers", "sum_totaal_groep")
indicator_variables <- c("n_totaal_gebruikers", "n_instellingen", "n_totaal_declaraties", "share_main_instelling")

dt_results <- rbindlist(lapply(split(dt_mszprest_DBC_agg, by = c("bin_size", "cohort", "died")), function(dt) {
  ...
  dt_ranked_by_variable <- rbindlist(lapply(ranking_variables, function(ranking_var) {
    ...
    dt_ranked <- dt_ranked[, ':='(
      ...
      ranked_by = paste(split_bin_size, split_cohort, split_died, ranking_var, sep = "_")
    )][ranking <= 50]
    ...
    for (comparison_var in names(vars_to_compare)) {
      ...
      dt_temp <- copy(dt_mszprest_DBC_agg)
      ...
      dt_ranked <- merge(
        dt_ranked, 
        dt_temp[, .SD, .SDcols = c("vektmszdbczorgproduct", opposite_col_names)], 
        by = "vektmszdbczorgproduct",
        all.x=T
      )
    }
...
reflijst_zorgproducten <- load_reflijst_zorgproducten(c(
  "mszdbczorgproduct", "mszdbczorgproductomschrijving"
))
dt_results[, vektmszdbczorgproduct := as.numeric(vektmszdbczorgproduct)]

dt_results <- merge_with_validate(
  dt_results,
  reflijst_zorgproducten,
  by.x = "vektmszdbczorgproduct",
  by.y = "mszdbczorgproduct",
  require_match = "left",
...
openxlsx2::write_xlsx(list(
  "figures" = dt_results,
  "invalid_counts" = invalid_counts
), "data/processed/top50_mszprest_DBC_codes_by_category.xlsx")

#### 5: create top 50 tables for activiteiten ####
# load in datasets
...
indicator_variables <- c("n_totaal_gebruikers", "n_instellingen", "n_totaal_activiteiten")
